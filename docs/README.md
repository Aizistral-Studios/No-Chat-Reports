...
1. **Only on the client**: The client will refuse to send the account's public key to the server, and signatures will be stripped from the messages that are sent. This way it won't be possible to report your messages, as there will be no proof they were actually sent from your account. The server will relay them unless the "enforce-secure-profile" option is set to true (which it is by default since 1.19.1), in which case you will not be able to join unless you agree to send signed messages (the mod will display a warning screen).
